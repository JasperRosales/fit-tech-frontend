// Enhanced Product Operations for Cloth Store
// ===========================================

import { mockProducts, mockCategories, getCategoryName } from '../clothStoreData';
import { 
  initializeStorage, 
  getProducts, 
  saveProducts, 
...
export const getAllProducts = (params = {}) => {
  ...
  const limit = parseInt(params.limit) || 12; // Show 12 products per page for cloth store
...
export const getProductsByCategory = (categoryId, limit = null) => {
  ...
  let filteredProducts = products.filter(p => 
    p.category_id === parseInt(categoryId) && p.is_active
  );
...
export const getDiscountedProducts = (limit = null) => {
  const products = getProducts();
  let discountedProducts = products.filter(p => 
    p.discount > 0 && p.is_active
  );
...
export const searchProducts = (query, limit = 20) => {
  ...
  const searchResults = products.filter(p => 
    p.is_active && (
      p.name.toLowerCase().includes(searchTerm) ||
      p.description.toLowerCase().includes(searchTerm) ||
      getCategoryName(p.category_id).toLowerCase().includes(searchTerm)
    )
  );
